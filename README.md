...
1. 確認電腦已安裝Node.js。
2. 在根資料夾輸入npm install。
3. 輸入npm run dev。
(API Key 與 Config 已在程式碼中)
...
* 通知
如果被傳訊息的分頁在背景開著(不是無痕模式)，就會在右下角跳出Chrome通知。
* 動畫
新的訊息會以由下往上浮出的方式出現在聊天室，彈出視窗也都會有動畫。
* User Profile
...
滑鼠移至訊息上方可以回覆訊息，傳送後點擊回覆的訊息，會自動滑至該訊息的位置，且該訊息會左右晃動。
* 其他功能
深色模式(左上太陽按鈕)、顯示訊息時間
